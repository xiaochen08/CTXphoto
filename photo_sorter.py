...
def main_ui():
    ...
    accent.grid_propagate(False)
    title_var = tk.StringVar(value="照片/视频导入与分类")
    ttk.Label(header, textvariable=title_var, style="AuroraTitle.TLabel").grid(row=1, column=0, sticky="w")

    nav_btn = ttk.Button(header, text="检测废片", style="AuroraPrimary.TButton")
    nav_btn.grid(row=1, column=1, sticky="e", padx=(24, 10))

    ttk.Label(header, text="主题", style="AuroraBodyOnPanel.TLabel").grid(row=1, column=2, sticky="e", padx=(24, 10))
    theme_box = ttk.Combobox(header, state="readonly", values=THEMES, width=8, style="Aurora.TCombobox")
    theme_box.grid(row=1, column=3, sticky="e")
    theme_box.set(THEMES[0])

    view_stack = ttk.Frame(root, style="AuroraPanel.TFrame")
    view_stack.grid(row=1, column=0, sticky="nsew", padx=36, pady=(0, 28))
    view_stack.grid_rowconfigure(0, weight=1)
    view_stack.grid_columnconfigure(0, weight=1)

    main_frame = ttk.Frame(view_stack, style="AuroraPanel.TFrame")
    main_frame.grid(row=0, column=0, sticky="nsew")
    main_frame.grid_columnconfigure(0, weight=1)
    main_frame.grid_columnconfigure(1, weight=0)
    main_frame.grid_rowconfigure(0, weight=1)

    waste_view = WasteDetectionView(view_stack, root, theme_key, on_back=None)
    waste_view.frame.grid(row=0, column=0, sticky="nsew")
    main_frame.tkraise()
    ...
    def show_primary_view():
        title_var.set("照片/视频导入与分类")
        main_frame.tkraise()
        nav_btn.config(text="检测废片", style="AuroraPrimary.TButton", command=show_waste_view)

    def show_waste_view():
        title_var.set("闭眼/曝光检测与筛选")
        waste_view.show()
        waste_view.frame.tkraise()
        nav_btn.config(text="返回导入界面", style="AuroraGhost.TButton", command=show_primary_view)

    nav_btn.config(command=show_waste_view)
    waste_view.on_back = show_primary_view
    ...
    def on_theme_change(event=None):
        ...
        waste_view.apply_theme(DEFAULT_THEME_KEY)
