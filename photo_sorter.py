...
# - 主界面栅格化重构，统一按钮尺寸与间距，缺失依赖时提供一次性横幅提示
...
DEPENDENCY_STATE = SimpleNamespace(
    has_mediapipe=mp is not None,
    has_send2trash=send2trash is not None,
    detection_ready=(mp is not None and cv2 is not None and np is not None),
    banner_dismissed=False,
)


def detection_dependencies_ready() -> bool:
    return DEPENDENCY_STATE.detection_ready
...
def rollback_files(paths, target_root):
    ...
    trash_available = DEPENDENCY_STATE.has_send2trash and send2trash is not None
...
class HoverTooltip:
    ...
    def _show(self, event=None):
        ...
        tw.configure(bg="#1B273A")
        label = ttk.Label(tw, text=self.text, style="AuroraBodyOnPanel.TLabel")
        label.configure(background="#1B273A", foreground="#E2E8F0", padding=(10, 6))
        label.pack()
        try:
...
def show_dependency_help(parent=None):
    parts = []
    if not DEPENDENCY_STATE.has_mediapipe:
        parts.append("mediapipe 未安装：可执行 pip install mediapipe 来启用闭眼与曝光检测模块。")
    if not DEPENDENCY_STATE.has_send2trash:
        parts.append("send2trash 未安装：可执行 pip install send2trash 以启用安全回收站删除。")
    if not parts:
        parts.append("所有可选依赖已就绪，无需额外操作。")
    aurora_showinfo("依赖说明", "\n".join(parts), parent=parent)
...
def start_copy(
    ...
    def worker():
        ...
        def finalize():
            ...
            if detect_btn is not None:
                set_button_state(detect_btn, active=detection_dependencies_ready())
...
def main_ui():
    ...
    missing_msgs = []
    if not DEPENDENCY_STATE.has_mediapipe:
        missing_msgs.append("未检测到 mediapipe，闭眼/曝光检测功能不可用")
    if not DEPENDENCY_STATE.has_send2trash:
        missing_msgs.append("未检测到 send2trash，回收站删除不可用")

    row_cursor = 0
    banner_frame = None

    if missing_msgs and not DEPENDENCY_STATE.banner_dismissed:
        banner_frame = ttk.Frame(root, style="AuroraHeader.TFrame", padding=(24, 12))
        banner_frame.grid(row=row_cursor, column=0, sticky="ew", padx=36, pady=(24, 0))
        banner_frame.grid_columnconfigure(0, weight=1)

        ttk.Label(
            banner_frame,
            text="；".join(missing_msgs),
            style="AuroraBodyOnPanel.TLabel",
            anchor="w",
        ...
        ttk.Button(
            ...
            command=lambda: show_dependency_help(root),
        ).grid(row=0, column=1, padx=(16, 0))

        def _dismiss_banner():
            banner_frame.grid_remove()
            DEPENDENCY_STATE.banner_dismissed = True
    ...
    header_pady_top = 28 if row_cursor == 0 else 16
    header.grid(row=row_cursor, column=0, sticky="ew", padx=36, pady=(header_pady_top, 16))
    ...
    main_frame.grid(row=row_cursor, column=0, sticky="nsew", padx=36, pady=(0, 28))
    root.grid_rowconfigure(row_cursor, weight=1)
    main_frame.grid_columnconfigure(0, weight=1)
    main_frame.grid_columnconfigure(1, weight=1)
    main_frame.grid_rowconfigure(0, weight=1)

    log_frame = ttk.Frame(main_frame, style="AuroraPanel.TFrame", width=LOG_PANEL_WIDTH)
    log_frame.grid(row=0, column=1, sticky="nsew", padx=(16, 0))
    log_frame.grid_propagate(False)
    log_frame.grid_rowconfigure(0, weight=1)

    log_card = ttk.Frame(log_frame, style="AuroraCard.TFrame", padding=(28, 24))
    ...
    left_container = ttk.Frame(main_frame, style="AuroraPanel.TFrame")
    left_container.grid(row=0, column=0, sticky="nsew", padx=(0, 16))
    left_container.grid_columnconfigure(0, weight=1)
    left_container.grid_rowconfigure(0, weight=1)

    form_card = ttk.Frame(left_container, style="AuroraCard.TFrame", padding=(28, 26))
    ...
    ttk.Label(form_card, text="导入设置", style="AuroraSection.TLabel").grid(row=0, column=0, columnspan=3, sticky="w", pady=(0, 16))

    ttk.Label(form_card, text="选择素材盘", style="AuroraBody.TLabel").grid(row=1, column=0, sticky="e", padx=(0, 12))
    combo_src = ttk.Combobox(form_card, state="readonly", width=70, style="Aurora.TCombobox")
    combo_src.grid(row=1, column=1, sticky="ew")
    ttk.Button(form_card, text="刷新", style="AuroraGhost.TButton",
               command=lambda: refresh_sources(info_box, combo_src, auto_pick=True)).grid(row=1, column=2, padx=(14, 0))

    ttk.Label(form_card, text="接入到", style="AuroraBody.TLabel").grid(row=2, column=0, sticky="e", padx=(0, 12), pady=(14, 0))
    combo_dst = ttk.Combobox(form_card, state="readonly", width=50, style="Aurora.TCombobox")
    ...
    ttk.Label(form_card, text="拍摄类型", style="AuroraBody.TLabel").grid(row=3, column=0, sticky="e", padx=(0, 12), pady=(14, 0))
    combo_cat = ttk.Combobox(form_card, state="readonly", values=CATEGORIES, width=20, style="Aurora.TCombobox")
    combo_cat.grid(row=3, column=1, sticky="w", pady=(14, 0))
    combo_cat.current(0)

    def _standardize_button(btn: ttk.Button):
        btn.config(width=14)

    extract_star_enabled = False
    star_btn: Optional[ttk.Button] = None
    ...
    star_btn = ttk.Button(form_card, text="提取星标照片 ×", style="AuroraPrimary.TButton", command=toggle_star)
    star_btn.grid(row=4, column=0, columnspan=2, sticky="w", pady=(18, 0))
    _standardize_button(star_btn)
    refresh_star_button_visual()
    ...
    detect_btn.grid(row=4, column=2, sticky="e", pady=(18, 0))
    _standardize_button(detect_btn)

    progress_value_var = tk.DoubleVar(value=0.0)
    progress_pct_var = tk.StringVar(value="进度: 0.0%")
    eta_var = tk.StringVar(value="剩余时间：--")
    progress_display_var = tk.StringVar()

    def _sync_progress_caption(*_):
        progress_display_var.set(f"{progress_pct_var.get()}    {eta_var.get()}")

    _sync_progress_caption()
    progress_pct_var.trace_add("write", _sync_progress_caption)
    eta_var.trace_add("write", _sync_progress_caption)

    progress_frame = ttk.Frame(form_card, style="AuroraCard.TFrame")
    progress_frame.grid(row=5, column=0, columnspan=3, sticky="ew", pady=(18, 0))
    progress_frame.grid_columnconfigure(0, weight=1)

    status_lbl = ttk.Label(progress_frame, text="待机", style="AuroraStatus.TLabel")
    ...
    progress_info_lbl = ttk.Label(progress_frame, textvariable=progress_display_var, style="AuroraStatus.TLabel")
    progress_info_lbl.grid(row=1, column=1, sticky="e", padx=(16, 0))

    buttons_frame = ttk.Frame(form_card, style="AuroraCard.TFrame")
    buttons_frame.grid(row=6, column=0, columnspan=3, sticky="ew", pady=(24, 0))
    for col in range(4):
        buttons_frame.grid_columnconfigure(col, weight=1)
    buttons_frame.grid_columnconfigure(4, weight=0)

    def progress_start(_total=None):
        state.anim_running = True
        state.anim_cur = 0.0
        state.anim_tgt = 0.0
        state.last_real_update = time.monotonic()
        state.last_sim_bump = state.last_real_update
        state.progress_start_time = state.last_real_update
        progress_value_var.set(0.0)
        progress_pct_var.set("进度: 0.0%")
        eta_var.set("剩余时间：计算中…")
        state.last_eta_text = "剩余时间：计算中…"
        if _total is not None:
            try:
                state.total_bytes = max(0, int(_total))
            except Exception:
                pass
    ...
    def progress_set_real(copied, total):
        state.last_real_update = time.monotonic()
        if total <= 0:
            state.anim_tgt = min(99.0, state.anim_cur + 0.3)
            return
        percent = (copied / total) * 100.0
        state.anim_tgt = max(min(percent, 99.0), 0.0)

    def progress_bump_sim(pace=0.3, cap=99.0):
        now = time.monotonic()
        if (now - state.last_sim_bump) < 0.4:
            return
    ...
    def set_detect_button_state():
        available = detection_dependencies_ready()
        set_button_state(detect_btn, active=available)
        if not available:
            if state.detect_tooltip is None:
                state.detect_tooltip = HoverTooltip(detect_btn, "安装 mediapipe 后可用")
        else:
            state.detect_tooltip = None
    ...
    def on_cancel():
        if not state.is_copying:
            return
        if DEPENDENCY_STATE.has_send2trash:
            if not aurora_askyesno(
                "确认取消",
                "还没复制完，是否取消？\n（取消会撤回本次所有未复制完成的文件）",
                parent=root,
            ...
            if not aurora_askyesno(
                "确认直接删除",
                "未检测到 send2trash，取消将直接删除本次已复制的文件，且无法回收。\n是否继续？",
                parent=root,
            ):
                return
            if not aurora_askyesno(
                "二次确认",
                "将直接删除已复制文件并清理当天目录，确定执行？",
                parent=root,
            ):
                return
        set_button_state(cancel_btn, active=False, style_active="AuroraDanger.TButton")
    ...
    btn_start = ttk.Button(buttons_frame, text="开始分类", style="AuroraPrimary.TButton", command=start_action)
    btn_start.grid(row=0, column=0, sticky="ew", padx=(0, 12))
    _standardize_button(btn_start)

    open_btn = ttk.Button(buttons_frame, text="打开文件夹", style="AuroraPrimary.TButton", command=open_current_month)
    open_btn.grid(row=0, column=1, sticky="ew", padx=(0, 12))
    _standardize_button(open_btn)

    pause_btn = ttk.Button(buttons_frame, text="暂停", style="AuroraPrimary.TButton", command=on_pause, state="disabled")
    pause_btn.grid(row=0, column=2, sticky="ew", padx=(0, 12))
    _standardize_button(pause_btn)

    cancel_label_text = "取消" if DEPENDENCY_STATE.has_send2trash else "直接删除"
    cancel_btn = ttk.Button(buttons_frame, text=cancel_label_text, style="AuroraPrimary.TButton", command=on_cancel, state="disabled")
    cancel_btn.grid(row=0, column=3, sticky="ew")
    _standardize_button(cancel_btn)

    trash_notice_label = ttk.Label(buttons_frame, text="无回收站（未安装 send2trash）", style="AuroraStatus.TLabel")
    if not DEPENDENCY_STATE.has_send2trash:
        trash_notice_label.grid(row=1, column=0, columnspan=4, sticky="w", pady=(8, 0))

    set_button_state(star_btn, active=True)
    set_detect_button_state()
    set_button_state(btn_start, active=True)
    ...
    row_cursor += 1
    footer.grid(row=row_cursor, column=0, pady=(0, 18))
